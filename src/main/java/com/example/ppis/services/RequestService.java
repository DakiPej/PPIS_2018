...
public class RequestService {
	...
	@Autowired
	public void setDAOs(RequestDAO requestDao, RegisteredUserDAO registeredUserDao, ContactMethodDAO contactMethodDao,
			DepartmentDAO departmentDao, RequestLogDAO requestLogDao) {
	...
	private boolean validateNewRequest(String registeredUserUsername, String contactMethod, String title,
			String description, Integer urgency) {
	...
	private boolean validateReassign(long requestId, String departmentName, Integer priority) {
		...
		if (requestId < 0 || !this.requestDao.existsById(requestId))
			throw new IllegalArgumentException(
					"The request id is either a negative number or the request does not exist.");
	...
	private boolean validateUnassignedRequestToResolver(long requestID, String resolverUsername) {
		...
		if (resolverUsername.length() == 0 || !this.registeredUserDao.existsByUsername(resolverUsername))
			throw new IllegalArgumentException(
					"The resolver username is not either unspecified or the user does not exist.");

		return true;
	}

	public String saveNewRequest(String registeredUserUsername, String contactMethod, String title, String description,
			Integer urgency) {
		...
		try {
			...
			if (validateNewRequest(registeredUserUsername, contactMethod, title, description, urgency)) {

				if (contactMethod.equals("email"))
					request = new Request(this.registeredUserDao.findUserByUsername(registeredUserUsername),
							this.contactMethodDao.getContactMethodByName(contactMethod), title, description, urgency,
				...
				RequestLog log = new RequestLog(this.requestDao.create(request), new Date(), "Request created");
	...
	public String assingRequestToAdmin(String username, Long requestId) {
		try {
			if (username == null || username.length() == 0 || requestId == null || requestId < 0
					|| !this.requestDao.existsById(requestId))
				throw new IllegalArgumentException(
						"The user is not specified or does not exist or the request id is not valid or the request does not exist.");
			;
	...
	public String assignRequestToResolver(long unassignedRequestId, String resolverUsername) {
		try {
			if (validateUnassignedRequestToResolver(unassignedRequestId, resolverUsername)) {
				...
				RequestLog log = new RequestLog(this.requestDao.create(unassignedRequest), new Date(), "The user : "
						+ unassignedRequest.getResolverUser().getUsername() + " will handle the request.");
	...
	public String escalateRequest(long requestId, String resolverUsername) {
		try {
			if (requestId < 0 || !this.requestDao.existsById(requestId) || resolverUsername == null
					|| resolverUsername.length() == 0 || this.registeredUserDao.existsByUsername(resolverUsername))
				throw new IllegalArgumentException(
			...
			if (this.registeredUserDao.findUserByUsername(resolverUsername).getUserType().equals("Korisnik")
					|| !this.requestDao.one(requestId).getResolverUser()
							.equals(this.registeredUserDao.findUserByUsername(resolverUsername)))
			...
			RequestLog log = new RequestLog(this.requestDao.create(request), new Date(), "The request has escalated.");
			this.requestLogDao.create(log);
	...
	public String reassignRequest(long requestId, String departmentName, Integer priority) {
		...
		try {
			...
			if (validateReassign(requestId, departmentName, priority)) {
				...
				RequestLog log = new RequestLog(this.requestDao.create(reassignedRequest), new Date(),
	...
	public String closeRequest(long requestId, String registeredUsername) {
		try {
			if (requestId < 0 || !this.requestDao.existsById(requestId) || registeredUsername == null
					|| registeredUsername.length() == 0 || !this.registeredUserDao.existsByUsername(registeredUsername))
				throw new IllegalArgumentException(
			...
			RequestLog log = new RequestLog(this.requestDao.create(closedRequest), new Date(),
	...
	public List<Request> getRequestsByRegisteredUser(String registeredUserUsername) {
		...
		try {
			...
			if (registeredUserUsername.length() > 0
					&& this.registeredUserDao.existsByUsername(registeredUserUsername)) {
				requests = this.requestDao
						.getRequestsByRegisteredUser(this.registeredUserDao.findUserByUsername(registeredUserUsername));
			} else
				throw new IllegalArgumentException("The username is either unspecified or the user has no requests.");
	...
	public List<Request> getRequestsByAdmin(String admin) {
		...
		try {
			if (admin == null
					|| (admin != null && admin.length() != 0 && this.registeredUserDao.existsByUsername(admin)))
	...
	public List<Request> getRequestsByResolver(String resolverUsername) {
		...
		try {
			if (resolverUsername == null
					|| resolverUsername.length() > 0 && this.registeredUserDao.existsByUsername(resolverUsername))
				requests = this.requestDao
						.getRequestsByResolver(this.registeredUserDao.findUserByUsername(resolverUsername));
			else
				throw new IllegalArgumentException("The username is either unspecified or the user has no requests.");
		} catch (Exception e) {
			throw e;
	...
	public List<Request> getRequestByRegisteredUserAndResolverUser(String registeredUsername, String resolverUsername) {
		...
		try {
			if (registeredUsername == null || registeredUsername.length() == 0
	...
	public List<Request> getRequestsByResolverUserAndGreaterPriority(String resolverUsername, int priority) {
		...
		try {
			if (priority > 0 && this.registeredUserDao.existsByUsername(resolverUsername))
				...
				throw new IllegalArgumentException(
						"The priority is less or equal to zero or the resolver is either unspecified or does not exist.");
		} catch (Exception e) {
			throw e;
		}
	...
	public List<Request> getRequestByRegisteredUserAndGreaterUrgency(String registeredUser, int urgency) {
		...
		try {
			if (urgency > 0 && registeredUser.length() > 0 && this.registeredUserDao.existsByUsername(registeredUser))
				...
				throw new IllegalArgumentException(
						"The urgency is a negative number or the user is unspecified or doesn't exist.");
	...
	/*
	 * public List<Request> getRequestsByRegisteredUserAndResolved(String
	 * registeredUser, boolean resolved) {
	 * 
	 * List<Request> requests ;
	 * 
	 * try { if(registeredUser.length() > 0 &&
	 * this.registeredUserDao.existsByUsername(registeredUser)) requests =
	 * this.requestDao.getRequestsByRegisteredUserAndResolved(
	 * this.registeredUserDao.findUserByUsername(registeredUser), resolved) ; else
	 * throw new
	 * IllegalArgumentException("The user is either unspecified or does not exist.")
	 * ; } catch (Exception e) { throw e ; }
	 * 
	 * return requests ; } public List<Request>
	 * getRequestsByResolverUserAndResolved(String resolverUser, boolean resolved) {
	 * 
	 * List<Request> requests ;
	 * 
	 * try { if(resolverUser.length() > 0 &&
	 * this.registeredUserDao.existsByUsername(resolverUser)) requests =
	 * this.requestDao.getRequestsByResolverUserAndResolved(
	 * this.registeredUserDao.findUserByUsername(resolverUser), resolved) ; else
	 * throw new
	 * IllegalArgumentException("The user is either unspecified or does not exist.")
	 * ; } catch (Exception e) { throw e ; }
	 * 
	...
	public List<Request> getRequestsByRegisteredUserAndClosed(String registeredUser, boolean closed) {
		...
		try {
			if (registeredUser.length() > 0 && this.registeredUserDao.existsByUsername(registeredUser))
				...
				throw new IllegalArgumentException("The user is either unspecified or does not exist.");
		} catch (Exception e) {
			throw e;
		}
	...
	public List<Request> getRequestsByDepartment(String department) {
		...
		try {
			if (department == null || department.length() > 0 && this.departmentDao.existsByDepartmentName(department))
				...
				throw new IllegalArgumentException("The department name is either unspecified or does not exist.");
		} catch (Exception e) {
			throw e;
		}
	...
	public List<Request> getRequestsByResolverUserAndTitle(String resolverUser, String title) {
		...
		try {
			if (title.length() > 0 && resolverUser.length() > 0
					&& this.registeredUserDao.existsByUsername(resolverUser))
				requests = this.requestDao.getRequestsByResolverUserAndTitle(
						this.registeredUserDao.findUserByUsername(resolverUser), title);
			else
				throw new IllegalArgumentException(
						"The title unspecified or the resolver username is either unspecified or the user does not exist,");
	...
	public List<Request> getAll(String username) {
		try {
			...
			if (user.getUserType().getTypeName().equals("Administrator")) {
				...
				requests = this.requestDao.getRequestsByResolver(user);
			return requests;
		} catch (Exception e) {
			throw e;
		}
